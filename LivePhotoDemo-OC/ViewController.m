...
- (void)loadVideoWithVideoURL:(NSURL *)videoURL {
    ...
    [generator generateCGImagesAsynchronouslyForTimes:@[time] completionHandler:^(CMTime requestedTime, CGImageRef  _Nullable image, CMTime actualTime, AVAssetImageGeneratorResult result, NSError * _Nullable error) {
        if (image) {
            ...
            NSURL *imageURL = [urls.firstObject URLByAppendingPathComponent:@"image.jpg"];
            ...
            if (!error) {
                [[NSFileManager defaultManager] removeItemAtPath:[NSString stringWithFormat:@"%@/IMG.JPG", output] error:&error];
                [[NSFileManager defaultManager] removeItemAtPath:[NSString stringWithFormat:@"%@/IMG.MOV", output] error:&error];
            }
            
            [[[JPEG alloc] initWithPath:image] writeToDest:[NSString stringWithFormat:@"%@/IMG.JPG", output] withAssetIdentifier:assetIdentifier];
            ...
            dispatch_sync(dispatch_get_main_queue(), ^{
                [PHLivePhoto requestLivePhotoWithResourceFileURLs:@[[NSURL fileURLWithPath:[[ViewController livePath] stringByAppendingString:@"/IMG.MOV"]], [NSURL fileURLWithPath:[[ViewController livePath] stringByAppendingString:@"/IMG.JPG"]]] placeholderImage:nil targetSize:self.view.bounds.size contentMode:PHImageContentModeAspectFit resultHandler:^(PHLivePhoto * _Nullable livePhoto, NSDictionary * _Nonnull info) {
...
- (void)exportLivePhoto {
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        [createRequest addResourceWithType:PHAssetResourceTypePhoto fileURL:[NSURL fileURLWithPath:[[ViewController livePath] stringByAppendingString:@"/IMG.JPG"]] options:options];
